...
> 本文档说明项目整体设计（模型架构、训练策略、评估协议）；**特征定义见 [FEATURES_V34.md](FEATURES_V34.md)**，
> **运行步骤见 [QUICKSTART.md](QUICKSTART.md)**，**实验结果见 [EXPERIMENT_RESULTS.md](EXPERIMENT_RESULTS.md)**。
> （历史完整说明归档于 `docs/archive/PROJECT_EXPLANATION.md`，如需早期细节可查。）

## 1. 任务与数据

- **任务**：三峡库区消落带滑坡空间易发性建模——对斜坡单元预测"是否易发"（0/1）；
- **训练人群**：25,636 个斜坡单元（全量 26,068 剔除 432 个常年水下单元——河道/库底不可能滑坡，不参与训练；全图推理时回填 prob=0/极低级，保证出图无空缺）；
- **标签**：研究期 2003-2021 内首次发生滑坡的单元为正样本（661 个，2.6%），其余为负；
- **特征**：34 维环境因子（见 FEATURES_V34.md），事件前窗口防泄漏口径。
...
### 4.2 负采样三口径（详见 FEATURES_V34.md §四）
...
- **为何不用 SMOTE**：滑坡样本是真实斜坡单元的统计特征，特征空间插值造出的"合成坡"物理上不存在，且无法纠正"负样本混入远山"的偏差（见 FEATURES_V34.md §4.4）。
...
predict_gnn.py         全图推理出图（训练人群推理 + 水下单元回填）
predict_xgb_v30.py     XGBoost 全图推理（同方案 C 回填逻辑）
src/config.py          路径 + 34 维特征定义 + 超参数（INPUT_DIM=34）
