...
# 或分阶段执行
python main.py --stage data       # 特征提取 → 对照表 + 事件窗口 34 维主线特征表（见 docs/FEATURES_V34.md）
python main.py --stage graph      # 图构建
python main.py --stage baseline   # XGBoost 基线（34 维主线，AUC≈0.82）
...
# 1) 各特征提取（静态对照口径）
python tills/extract_terrain_features.py    # → terrain_features.csv（地形均值）
python tills/extract_water_features.py      # → water_features.csv（淹没 inundation_fraction）
python tills/extract_water_network_features.py  # → 水系距离/密度
python tills/extract_landuse_features.py    # → CLCD 土地利用年度矩阵
python tills/extract_road_features.py       # → 道路特征（需 OSM 下载到 data/roads/）
python tills/merge_features.py              # → features/features.csv（静态对照表 + 标签）

# 2) ★ 干流逐日水位 → 干湿循环特征（评审新增，新水位数据）
python tills/parse_station_water.py         # → daily_water_levels.csv + station_assign.csv
python tills/extract_wetdry_features.py     # → wetdry_static_features.csv（静态干湿）
python tills/extract_wetdry_event_features.py  # → wetdry_all_features.csv（静态+事件前）

# 3) ★ 事件窗口 34 维主线特征表（见 docs/FEATURES_V34.md）
python tills/build_event_window_features.py --k 2 --start-year 2000 --seed 42   # → 24 维中间表
python tills/build_v30_features.py --k 2    # → 30 维基础表（+道路4 +土地利用变化3，-冗余 curvature）
# → 30 维全量表 + 干湿 4 维 = 34 维（当前主线，见 merge_wetdry_features.py）
#   训练表 features/event_window_features_k2_v34_train.csv（25636 × 35）
#   全量表 features/event_window_features_k2_v34.csv（26068 × 35，供全图推理）
#   正样本 T=首次滑坡年；负样本 T=频率匹配伪事件年；时序特征只用 [2000, T-1]（无泄漏）
```

### 4.2 图构建

```bash
python tills/build_graph.py --method polygon_adjacency   # 共享边界邻接（推荐，输出 graph_train.npz 25636 节点）
...
# 查看 results/baseline_xgb_ew_feat_k2_v34_train_madmin.json：平均 AUC 与特征重要性
# 当前实测：AUC ≈ 0.8233 ± 0.0234（34 维主线，训练人群 25636，按县分折 × 全域）
# 负采样参考跑（admin 分折，记录于 EXPERIMENT_RESULTS.md §3.1e）：
python baseline_xgb.py --features-csv features/event_window_features_k2_v34_train.csv \
    --folds 5 --method admin --neg-sampling soft --neg-km 4 --neg-lam 0.2   # 软采样 0.8131 / 池 0.7861
python baseline_xgb.py --features-csv features/event_window_features_k2_v34_train.csv \
    --folds 5 --method admin --neg-sampling proximity --neg-km 4 --neg-k 2  # 硬采样 0.7944 / 池 0.7354
...
# ★ 上云 GPU 训练（正式 B 主实验）：服务器规格/打包/环境/命令/带回清单见 docs/SERVER_TRAIN.md
```
...
| 基线 AUC 异常高（接近 1.0） | 存在特征泄漏：检查是否混入复发特征/截断类时序特征（防泄漏口径见 FEATURES_V34.md 各组"防泄漏"列） |
