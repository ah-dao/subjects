...
6. **负样本设计（评审问题 1）**：全域负样本（25222）改为**时空邻近策略**，提供硬采样与软采样两种口径，并用**采样池 AUC** 评估同环境判别力（详见 6.2 节）。
...
| 土地利用矩阵 | CLCD 30m（Albers），`landuse_unit_matrix.csv`（25884 单元 × 22 年 × 2 类占比） |
...
- 产物 `landuse_unit_matrix.csv`（25884 × 45，0 缺失），T−1 截断取值在 `build_event_window_features.py` 完成。
...
| 方式 | 做法 | 结果（XGBoost×全域） |
|------|------|----------------------|
| `spatial_kmeans`（默认） | 质心 K-Means 聚类成 5 折（消除空间自相关虚高） | 0.7099 ± 0.0226（19 维口径，24 维待重测） |
| `admin`（按县分折） | 按县级归属分折，贪心均衡各折正样本（论文规范） | **0.8068 ± 0.0211（24 维）** |
| `random` | 随机划分（对照，一般高估） | — |
| **跨县留出**（`cross_county_validate.py`） | 70/30 按正样本占比随机分县，测试县完全留出，多组取均值 | 0.7226 ± 0.0162（19 维口径，24 维待重测） |

> ⚠ 上表 24 维完整矩阵（KMeans/跨县 × 负采样三口径）待岩性特征接入后一并重跑。
...
**Q: 当前基线 AUC 是多少？**
24 维定稿（XGBoost 5 折 admin×全域）：**0.8068 ± 0.0211**。演进：19 维 0.7409 → +水系 0.7829 → +土地利用 0.8068。19 维口径的历史数字（KMeans×全域 0.7099、软采样 0.7375、跨县 0.7226 等）见 `docs/EXPERIMENT_RESULTS.md`；**24 维完整矩阵待岩性接入后重跑**。
