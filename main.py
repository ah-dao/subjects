...
用法：
    ...
    python main.py --stage data         # 仅数据准备（地形/时序/水位 → 28维特征表）
    python main.py --stage graph        # 仅图构建
    python main.py --stage baseline     # 仅 XGBoost 基线
...
def main():
    ...
    if args.stage in ('all', 'data'):
        ...
        run(tills / 'extract_temporal_features.py')   # NDVI/降雨 8 维（全窗口）
        run(tills / 'extract_water_features.py')      # 淹没 6 维
        run(tills / 'merge_features.py')              # 合并 22 维特征表 + 标签

    if args.stage in ('all', 'graph'):
        run(tills / 'build_graph.py')

    if args.stage in ('all', 'baseline'):
        run(ROOT / 'baseline_xgb.py', ['--folds', str(args.folds),
